...
train_R_nm = train_R_nm[which(clust_id==1),]
train_I_nm = train_I_nm[which(clust_id ==1),]
...
R_nm = test_R_nm[which(clust_id==1),]
...
#rename for cooperation with old code
I_nm = test_I_nm[which(clust_id==1),]
...
MAE 
#For the small cluster, this is equal to 86.68894 --- really bad
#might be because of small sample size?
#the U parameters tend to go to 0 while the V parameters explode 
#not sure why this happens...
#for the big cluster, this is equal to 7.829197, which is bad but better
#is the answer cluster size?

#the first 886 have a MAE of 6.898576
#the first 114 have a MAE of 5919.307
#so there are some sample size effects going on here?
...
#     post_R_nm[n,m] = sum(post_U_k_nd[[k]][id,] * post_V_k_md[[k]][m,])
#   }
# }
# R_nm = R_nm[which(I_nm>0)]
# post_R_nm = post_R_nm[which(I_nm>0)]
