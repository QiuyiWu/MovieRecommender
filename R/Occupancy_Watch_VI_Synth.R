...
true_kappa_kl = 2*(0.01412552/mean(true_kappa_kl)*true_kappa_kl)
...
O = length(which(true_W_nm>0)) #the number of movies actually rated
...
#initialize lambda_nm at a uniform distribution?
lambda_nm = matrix(0,nrow = N,ncol = M)
for(n in 1:N){
  for(m in 1:M){
    lambda_nm[n,] = rbeta(1,1,1)
  }
}
# # #initialize gamma_nm at truth
# lambda_nm = matrix(0,nrow = N,ncol = M)
# for(n in 1:N){
#   for(m in 1:M){
#     if(true_W_nm[n,m]==1){
...
#initailize mu_kl and nu_kl at random
mu_kl = matrix(0,nrow = K,ncol = L)
nu_kl = matrix(0,nrow = K,ncol = L)
for(k in 1:K){
  for(l in 1:L){
    num = 0
    for(n in 1:N){
      for(m in 1:M){
        if(true_U_n[n] == k & true_M_m[m] ==l & true_W_nm[n,m]==1){
          num = num + 1
        }
      }
    }
    mu_kl[k,l] = rgamma(1,num,2)
    nu_kl[k,l] = rgamma(1,num,2)
  }
}
# #initialize mu_kl and nu_kl at truth
# mu_kl = true_p_kl
# nu_kl = 1 - mu_kl
# for(k in 1:K){
#   for(l in 1:L){
...
#     }
#     
#     mu_kl[k,l] = num*mu_kl[k,l]
#     nu_kl[k,l] = num*nu_kl[k,l]
#   }
# }
...
for(it in 1:n_it){
  ...
  for(n in 1:N){
    for(k in 1:K){
      ...
      for(m in 1:M){
        for(l in 1:L){
          sum1 = lambda_nm[n,m]*(digamma(eta_kl[k,l]) - digamma(eta_kl[k,l]+iota_kl[k,l]))
          sum2 = (1-lambda_nm[n,m])*(digamma(iota_kl[k,l]) - digamma(eta_kl[k,l]+iota_kl[k,l]))
  ...
  for(k in 1:K){
    for(l in 1:L){
      ...
      for(n in 1:N){
        for(m in 1:M){
          eta_sum = eta_sum + lambda_nm[n,m]*delta_nk[n,k]*zeta_ml[m,l]
          iota_sum = iota_sum + (1-lambda_nm[n,m])*delta_nk[n,k]*zeta_ml[m,l]
        }
      }
  ...
  for(n in 1:N){
    for(m in 1:M){
      if(I_nm[n,m]==1){
        ...
        #find probability W_nm = 1 and W_nm = 0
        prob_s = 0
        prob_f = 0
        for(k in 1:K){
          for(l in 1:L){
            temp_s = delta_nk[n,k]*zeta_ml[m,l]
            temp_s = temp_s*(digamma(iota_kl[k,l])-digamma(eta_kl[k,l]+iota_kl[k,l])+digamma(mu_kl[k,l]) - digamma(mu_kl[k,l]+nu_kl[k,l]))
            prob_s = prob_s + temp_s
            
            temp_f = delta_nk[n,k]*zeta_ml[m,l]
            temp_f = temp_f*(digamma(iota_kl[k,l])-digamma(eta_kl[k,l]+iota_kl[k,l]))
            prob_f = prob_f + temp_f
          }
        }
        
        #normalize the probability of failure adn success on a log scale
        probs = c(prob_f,prob_s)
        probs = exp(probs - max(probs))/sum(exp(probs-max(probs)))
        lambda_nm[n,m] = probs[2]
      }
    }
  }
  ...
  for(k in 1:K){
    for(l in 1:L){
      ...
      for(n in 1:N){
        for(m in 1:M){
          mu_sum = mu_sum + I_nm[n,m]*delta_nk[n,k]*zeta_ml[m,l]*lambda_nm[n,m]
          nu_sum = nu_sum + (1-I_nm[n,m])*delta_nk[n,k]*zeta_ml[m,l]*lambda_nm[n,m]
        }
      }
...
mean(lambda_nm[I_nm==1])

# plot(true_W_nm,lambda_nm)
mean(lambda_nm[true_W_nm>0 & I_nm<1])
mean(lambda_nm[true_W_nm<1])